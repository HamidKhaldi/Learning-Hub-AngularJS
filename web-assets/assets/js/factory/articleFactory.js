...
scsFamilyApp.factory('articleData', function($http, $q){
	return {
		...
		getHeadlineDataTags: function(){
	    	return $http.get("siteUrl/_api/web/lists/getByTitle('Lst_Headlines-Data-Tags')/items?$select=*", { 
				headers: { "Accept": "application/json;odata=verbose" }
			})
			.then(function(response) {
				return response.data;
		...
		// getArticleChildData: function(articleList, articleName){
		// 	return $http.get("siteUrl/_api/web/lists/getByTitle('" + articleList + "')/items?$filter=Title eq'" + articleName + "'", { 
		// 		headers: { "Accept": "application/json;odata=verbose" }
		// 	})
		// 	.then(function(response) {
		// 		return response.data;
		...
		// },
		getArticleChildData: function(articleList, articleName){
			return $http.get("siteUrl/_api/web/lists/getByTitle('" + articleList + "')/items?$select=*&$top=5000", { 
				headers: { "Accept": "application/json;odata=verbose" }
			})
			.then(function(response) {
				return response.data;
